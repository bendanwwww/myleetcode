        ...
        for x in range(len(board)):
            for y in range(len(board[x])):
                if board[x][y] == word[0]:
                    if len(word) == 1:
                        return True
                    findMap = {str(x) + str(y): 1}
                    if self.find(word, board, 1, [x, y], findMap):
                        return True
        return False

    def find(self, word, board, wordIndex, boardIndex, findIndex):
        if wordIndex == len(word):
            return True
        x = boardIndex[0]
        y = boardIndex[1]
        if x > 0 and str(x - 1) + str(y) not in findIndex and board[x - 1][y] == word[wordIndex]:
            findIndexCopy = findIndex.copy()
            findIndexCopy[(str(x - 1) + str(y))] = 1
            if self.find(word, board, wordIndex + 1, [x - 1, y], findIndexCopy):
                return True
        if x < len(board) - 1 and str(x + 1) + str(y) not in findIndex and board[x + 1][y] == word[wordIndex]:
            findIndexCopy = findIndex.copy()
            findIndexCopy[str(x + 1) + str(y)] = 1
            if self.find(word, board, wordIndex + 1, [x + 1, y], findIndexCopy):
                return True
        if y > 0 and str(x) + str(y - 1) not in findIndex and board[x][y - 1] == word[wordIndex]:
            findIndexCopy = findIndex.copy()
            findIndexCopy[str(x) + str(y - 1)] = 1
            if self.find(word, board, wordIndex + 1, [x, y - 1], findIndexCopy):
                return True
        if y < len(board[x]) - 1 and str(x) + str(y + 1) not in findIndex and board[x][y + 1] == word[wordIndex]:
            findIndexCopy = findIndex.copy()
            findIndexCopy[str(x) + str(y + 1)] = 1
            if self.find(word, board, wordIndex + 1, [x, y + 1], findIndexCopy):
                return True
